...
from tkinter import Button, Label, Menu, Tk, Toplevel

import timeago

from sync_model import Status, SyncEventType
from sync_status import SyncStatus
from sync_view_style import COLUMN_PADDING, ROW_PADDING
...
class SyncView:
    def __init__(self, controller, model):
        ...
        self.sync_button = Button(
            self.master, text="Sync", command=self.sync_button_callback
        )
    ...
    def open_merge_popup(self):
        ...
        Button(
            self.popup_win, text="Use Local", command=self.use_local_button_callback
        ).grid(row=1, column=0, padx=20, pady=10)
        Button(
            self.popup_win, text="Use Cloud", command=self.use_cloud_button_callback
    ...
    def on_closing(self):
        if not self.trayMenu:
            ...
            self.master.tk.call(
                ...
                icon,
                "-callback",
                (self.master.register(self.menu_func), "%m", "%x", "%y"),
                "-pos",
                0,
                "-text",
                u"Addon Sync",
            ...
            self.trayMenu.add_command(
                label="Show Window", command=self.master.deiconify
            )
